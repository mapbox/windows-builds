...
::maybe iosjs.exe
IF NOT EXIST %LOCAL_NODE_EXE% (SET LOCAL_NODE_EXE=%PKGDIR%\node-v%NODE_VERSION%-%BUILDPLATFORM%\%BUILD_TYPE%\iojs.exe && SET LOCAL_NODE_PDB=%PKGDIR%\node-v%NODE_VERSION%-%BUILDPLATFORM%\%BUILD_TYPE%\iojs.pdb)
...
ECHO %LOCAL_NODE_EXE%
ECHO %LOCAL_NODE_PDB%
COPY %LOCAL_NODE_EXE% node.exe
IF %ERRORLEVEL% NEQ 0 GOTO ERROR
IF EXIST %LOCAL_NODE_PDB% COPY %LOCAL_NODE_PDB% node.pdb
IF %ERRORLEVEL% NEQ 0 GOTO ERROR
